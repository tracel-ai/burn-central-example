...
fn train<B: AutodiffBackend>(device: B::Device, config: MnistTrainingConfig) {
    ...
    println!("Training complete! Model saved to {}", ARTIFACT_DIR);
}
...
fn main() {
    type Backend = Autodiff<Cpu>;
    let device = Default::default();
    let config = MnistTrainingConfig::default();

    train::<Backend>(device, config);
}
